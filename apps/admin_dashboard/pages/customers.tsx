...
export default function CustomersPage() {
  ...
  const newThisMonth = users.filter((user) =>
    isCurrentMonth(user.joinDate),
  ).length;
  const churnRate =
    users.length === 0
      ? 0
      : (users.filter((user) => user.blocked).length / users.length) * 100;
  const averageLtv = useMemo(() => {
    const total = users.reduce(
      (sum, user) => sum + (user.lifetimeValue ?? 0),
      0,
    );
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="group rounded-2xl border border-white/5 bg-[#0D1117]/60 p-5 shadow-xl backdrop-blur-xl transition-all hover:bg-white/5">
            ...
            <div className="mt-6 h-1.5 w-full overflow-hidden rounded-full bg-white/5 border border-white/5">
              <div
                ...
                style={{
                  width: `${users.length === 0 ? 0 : Math.round((activeUsersCount / users.length) * 100)}%`,
                }}
              ></div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/5 bg-[#0D1117]/60 p-5 shadow-xl backdrop-blur-xl transition-all hover:bg-white/5">
            ...
            <p className="mt-4 text-[10px] text-white/40 font-bold uppercase tracking-wider">
          ...
          <div className="rounded-2xl border border-white/5 bg-[#0D1117]/60 p-5 shadow-xl backdrop-blur-xl transition-all hover:bg-white/5">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs font-bold text-white/20 ml-1 tracking-widest">
                STABLE
              </span>
            </div>
            <p className="mt-4 text-[10px] text-white/40 font-bold uppercase tracking-wider">
