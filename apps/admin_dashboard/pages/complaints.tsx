...
export default function ComplaintsPage() {
  ...
  const [filterMode, setFilterMode] = useState<"all" | "active" | "priority">(
    "all",
  );
  ...
  const openCount = complaints.filter(
    (complaint) => complaint.status.toLowerCase() === "open",
  ).length;
  ...
  const currentWeek = complaints.filter(
    (complaint) =>
      ...
      weekLabel(complaint.createdAt) === "current",
  ).length;
  const previousWeek = complaints.filter(
    (complaint) =>
      typeof complaint.createdAt === "number" &&
      weekLabel(complaint.createdAt) === "previous",
  ).length;
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-white/40 font-medium">
              Manage and resolve incoming water delivery issues and customer
              complaints.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 rounded-2xl bg-red-500/10 border border-red-500/20 p-5 backdrop-blur-xl">
            ...
            <div>
              ...
              <p className="mt-2 text-[10px] font-black uppercase tracking-[0.15em] text-red-400/60">
                Critical Open
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-2 rounded-2xl bg-[#0D1117]/60 border border-white/5 p-6 backdrop-blur-xl shadow-xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="text-right">
                <p
                  className={`text-xs font-black tracking-wider ${weeklyGrowthPercent >= 0 ? "text-red-400" : "text-emerald-400"}`}
                >
                  {weeklyGrowthPercent >= 0 ? "+" : ""}
                  {weeklyGrowthPercent.toFixed(0)}%
                </p>
                <p className="text-[10px] text-white/20 font-bold uppercase tracking-tighter">
                  vs Last Week
                </p>
              </div>
            </div>
            <div className="flex h-16 items-end gap-2 px-2">
              {weeklySeries.buckets.map((value, index) => (
                <div
                  ...
                  style={{
                    height: `${Math.max(15, (value / weeklySeries.maxValue) * 100)}%`,
                  }}
                ></div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="relative group overflow-hidden rounded-3xl bg-[#0D1117]/60 border border-white/5 p-8 lg:col-span-2 shadow-xl backdrop-blur-xl">
              ...
              <div className="relative z-10 flex h-full flex-col justify-center">
                ...
                <p className="mb-6 max-w-md text-sm text-white/40 font-medium leading-relaxed">
                  Your team improved ticket closure consistency this week.
                  Performance is tracking 12% above last month's baseline.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-between rounded-3xl bg-white/5 border border-white/5 p-8 backdrop-blur-xl">
              <div>
                ...
                <p className="mb-8 text-[10px] font-bold uppercase tracking-widest text-white/20">
              ...
              <div className="space-y-6">
                ...
                <div className="h-2 w-full rounded-full bg-white/5 border border-white/5 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${totalCount === 0 ? 100 : Math.max(0, Math.round((resolvedCount / totalCount) * 100))}%`,
                    }}
                  ></div>
                </div>
                <p className="text-[10px] italic text-white/20 font-medium leading-tight">
                  *Resolution index is calculated based on the last 50 processed
                  support tickets.
                </p>
              </div>
            </div>
