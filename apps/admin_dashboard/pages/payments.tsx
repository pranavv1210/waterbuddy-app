      ...
      <div className="space-y-10">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="text-4xl font-extrabold tracking-tight text-white">Treasury Management</h1>
            <p className="mt-1 text-white/40 font-medium">Real-time financial performance and seller settlements.</p>
          </div>
          <div className="flex gap-3">
            <button
              type="button"
              className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-6 py-2.5 text-xs font-bold text-white/60 transition-all hover:bg-white/10 hover:text-white"
            >
        ...
        {!loading && !error ? (
          <>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
                ...
                <div className="relative z-10 flex h-full flex-col justify-between">
                  <div>
                    <div className="flex items-start justify-between">
                      <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#14B8A6]">Total Revenue</span>
                  ...
                  <div className="mt-8 flex items-center gap-2 text-xs font-bold text-white/40 uppercase tracking-wider">
                    <span className="material-symbols-outlined text-[#14B8A6] text-lg">trending_up</span>
                    <span>₹{Math.round(summary.totalRevenue - previousEstimate)} growth vs last period</span>
                  </div>
                </div>
                <div className="absolute -bottom-12 -right-12 opacity-[0.03] group-hover:opacity-10 transition-opacity duration-1000">
                  <span className="material-symbols-outlined text-[240px]" style={{ fontVariationSettings: "'FILL' 1" }}>payments</span>
                </div>
              </div>

              <div className="rounded-3xl bg-white/5 border border-white/5 p-8 backdrop-blur-xl transition-all hover:bg-white/[0.07]">
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#14B8A6]">Platform Fee (15%)</span>
                <div className="mt-4 text-3xl font-black text-white tracking-tighter">{formatCurrency(commission)}</div>
                <div className="mt-8 h-2 w-full overflow-hidden rounded-full bg-white/5 border border-white/5">
                  <div className="h-full rounded-full bg-[#14B8A6] shadow-[0_0_15px_rgba(20,184,166,0.5)]" style={{ width: `${commissionTargetRatio}%` }}></div>
                </div>
              ...
              <div className="rounded-3xl bg-white/5 border border-white/5 p-8 backdrop-blur-xl transition-all hover:bg-white/[0.07]">
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-400">Pending Settlements</span>
                <div className="mt-4 text-3xl font-black text-white tracking-tighter">{formatCurrency(pendingPayoutAmount)}</div>
                <div className="mt-8 flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    <div className="h-2 w-2 rounded-full bg-[#14B8A6]"></div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-white/40">{completedPayoutCount} Finalized</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="h-2 w-2 rounded-full bg-amber-500"></div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-white/40">{pendingPayoutCount} Processing</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
              <div className="flex flex-col gap-8 rounded-3xl bg-[#0D1117]/60 border border-white/5 p-8 lg:col-span-2 backdrop-blur-xl shadow-xl">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-widest text-white">Revenue Dynamics</h3>
                    <p className="text-[10px] font-bold text-white/20 mt-1 uppercase tracking-widest">Performance Heatmap</p>
                  </div>
                ...
                <div className="flex min-h-[300px] flex-1 items-end justify-between gap-4 px-4 pb-4">
                  {weeklyRevenue.map((week) => {
                    ...
                    return (
                      <div key={week.label} className="group flex flex-1 flex-col items-center gap-3">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-white/20 group-hover:text-white/60 transition-colors">{week.label}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="flex flex-col justify-between rounded-3xl bg-[#14B8A6] p-8 text-white shadow-[0_0_30px_rgba(20,184,166,0.2)]">
                <div>
                  ...
                  <h3 className="text-xl font-extrabold tracking-tight">Payment Liquidity</h3>
                  <p className="mt-1 text-xs font-bold text-white/60 uppercase tracking-widest">Transaction Channel Split</p>
                </div>

                <div className="relative flex justify-center py-10">
                   <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center">
                        <div className="text-4xl font-black tracking-tighter">{onlineRatio.toFixed(0)}%</div>
                        <div className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Digital</div>
                      </div>
                   </div>
                ...
                <div className="space-y-4 pt-4 border-t border-white/10">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-wider">Digital Assets</span>
                    </div>
                    <span className="text-sm font-black">{formatCurrency(summary.onlineRevenue)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-2 w-2 rounded-full bg-white/30"></div>
                      <span className="text-[10px] font-bold uppercase tracking-wider">Physical Cash</span>
                    </div>
                    <span className="text-sm font-black">{formatCurrency(summary.codRevenue)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-3xl border border-white/5 bg-[#0D1117]/60 shadow-xl backdrop-blur-xl">
              <div className="flex items-center justify-between border-b border-white/5 p-8">
                <div>
                  <h3 className="text-lg font-black tracking-tight text-white">Settlement History</h3>
                  <p className="text-[10px] font-bold text-white/20 mt-1 uppercase tracking-[0.2em]">Partner Disbursement Logs</p>
                </div>
                <button
                  type="button"
                ...
                >
                  Full Archive <span className="material-symbols-outlined text-sm">arrow_forward</span>
                </button>
              </div>
                ...
                <table className="w-full border-collapse text-left">
                  <thead className="bg-white/5 text-[10px] uppercase tracking-[0.2em] text-white/40">
                    <tr>
                      ...
                      <th className="px-8 py-5 text-right font-bold">Registry</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5 text-sm">
                    {visiblePayouts.map((payout) => (
                      <tr key={payout.id} className="group transition-colors hover:bg-white/5">
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="font-bold text-white/90">{payout.sellerName}</div>
                              <div className="text-[10px] font-bold text-white/20 uppercase">Network ID: {payout.sellerCode}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-6 font-mono text-[10px] text-white/40 uppercase tracking-tighter">#{payout.transactionId}</td>
                        <td className="px-6 py-6 text-xs font-bold text-white/60 uppercase tracking-wider">{formatDate(payout.date)}</td>
                        <td className="px-6 py-6 text-base font-black text-white text-right tracking-tighter">{formatCurrency(payout.amount)}</td>
